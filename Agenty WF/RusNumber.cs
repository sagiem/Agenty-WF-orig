...
namespace Agenty_WF
{
    class RusNumber
    {
        ...
        public static string Kop(double d)
        {

            //while (!double.TryParse(Console.ReadLine(), out d) || d < 0 || d > 1);
            int i = ((int)(d * 100)) - ((int)d) * 100;
            //string prefix = i % 10 == 1 ? "копейка" : i % 10 < 5 && i % 10 != 0 ? "копейки" : "копеек";
            //string k = i.ToString()+" "+ prefix;
            return i.ToString();
